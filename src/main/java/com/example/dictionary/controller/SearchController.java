...
public class SearchController extends MainController {
    ...
    public void handleFavorAction(ActionEvent event) {
        ...
        boolean success = obj.addWord(word);

        if (success) {
            showSuccessAlert("Thêm từ thành công!");
        } else {
            showErrorAlert("Thêm từ thất bại.");
        }
    }
    ...
    private void displayWord(String word, String phenot) {
        ...
        htmlContent.append("<header><p><b>").append(word).append("  /").append(phenot).append("/").append("</b></p></header>");
    ...
    public void VoiceMute(ActionEvent event){
        ...
        String voi = api.getVoice(searchText);
        api.playAudio(voi);
    }
