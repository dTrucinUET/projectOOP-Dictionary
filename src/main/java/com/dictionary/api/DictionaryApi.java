...
public class DictionaryApi {
    public String getVoice(String word){
        try {
            ...
            if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                // Split response based on a specific character (e.g., newline)
                String[] parts = response.split("\"");
                int i = 0;
                for(i = 0; i < parts.length/2; i++){
                    if(parts[i].equals("audio")){
                        i += 2 ;
                        break;
                    }
                }
    ...
    public static void playAudio(String audioUrl) {
        // Phát âm thanh bằng JavaFX MediaPlayer
        Media media = new Media(audioUrl);
        MediaPlayer mediaPlayer = new MediaPlayer(media);
        mediaPlayer.play();
    }
    public static void main(String[] args) {
        String word = "hello"; // Thay thế từ cần tra cứu
        DictionaryApi a = new DictionaryApi();
        System.out.println(a.getVoice(word));
    }
        /*try {
